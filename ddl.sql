...
create table public.investment_project -- Ushbu jadvalda investitsiya loyihalari haqida ma'lumotlar saqlanadi (Tashkilotlardagi shtatlar soni staff_position jadvalidagi personal_limit ustunidagi qiymatlarning yig'indisidan olinishi kerak)
(
    id                           bigint       not null -- ID raqam (primary key)
        primary key,
    datetime_created             timestamp(6) not null, -- ma'lumot yaratilgan sana va vaqt
...
create table public.personal -- Ushbu jadvalda hodimlarning shaxsiy ma'lumotlari saqlanadi (Tashkilotlarda bo'yicha hodimlarning sonini chiqarish uchun staff_position_personal jadvalidan foydalanish kerak)
(
    ...
    organization_stir       varchar(255), -- hodimning tashkiloti STIR(inn) raqami (hodimlar va ularning qarindoshlari haqida ma'lumot olish uchun faqat ushbu stir raqamga tegishli tashkilotlarni olish kerak -> organization_stir in ('200599579', '206887857', '200941518', '200002933', '203621367', '309341717'))
...
create table public.staff_position -- Ushbu jadvalda ishchilarning lavozimlari haqida ma'lumotlar saqlanadi (Tashkilotlardagi shtatlar sonini aniqlash uchun ushbu jadvalning personal_limit dan tashkilotdagi ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak. ushbu query orqali chiqariladi: SELECT (SELECT SUM(sp.personal_limit) FROM public.staff_position sp WHERE sp.organization_stir = '200941518') - (SELECT COUNT(DISTINCT p.id) FROM public.personal p WHERE p.organization_stir = '200941518' AND p.status ILIKE '%Bo`shatilgan%');
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
    ...
    department_id     bigint -- bo'lim ID raqami (department jadvalining id ustuni bilan bog'langan)
        constraint fk2rccfqg11kksq37ku44aforg5
            references public.department,
    personal_limit    integer, -- lavozim limiti (tashkilotdagi shtatlar soni so'ralganda ushbu ustunni yig'indisidan ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak)
