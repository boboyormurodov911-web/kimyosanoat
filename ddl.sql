...
create table public.staff_position -- Ushbu jadvalda ishchilarning lavozimlari haqida ma'lumotlar saqlanadi (Tashkilotlardagi shtatlar sonini aniqlash uchun shunaqa tartibda chiqarish kerak chiqarish kerak -> SELECT SUM(sp.personal_limit) - (SELECT COUNT(DISTINCT p.id) FROM public.personal p WHERE p.status ILIKE '%Bo`shatilgan%') FROM staff_position sp)
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
