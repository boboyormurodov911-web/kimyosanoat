...
(
    ...
    ekim_country      varchar(255) -- eksport yoki import amalga oshirilgan mamlakat kodi (country_code jadvalidagi code ustuni bilan bog'langan)
    ...
    organization_tin  varchar(255) -- tashkilot TIN raqami, organization jadvalidagi inn ustuni bilan bog'langan (Tizim tashkilotlari so'ralganda shunchaki organization jadvalidagi inn ustuni bo'yicha JOIN qilib, hech qanaqa filterlarsiz umumiy tekshirish kerak: SELECT SUM(gg.value) FROM public.gov_goods gg JOIN public.organization o ON gg.organization_tin = o.inn. Asosiy 6ta tashkilot inn raqamini olmasdan hammasini tekshirish kerak)
...
create table public.legal_entity_debt -- Ushbu jadvalda yuridik shaxs yoki tashkilotlarning qarzlari haqida ma'lumotlar saqlanadi
(
    id               bigint       not null -- ID raqam (primary key)
        primary key,
    ...
    tin              varchar(255) -- qarzdor tashkilot TIN raqami
...
create table public.personal -- Ushbu jadvalda hodimlar haqida ma'lumotlar saqlanadi, ma'lumotlar lotincha, krillcha va Rus tilida bo'lishi mumkin
(
...
create table public.relatives -- Ushbu jadvalda qarindoshlar haqida ma'lumotlar saqlanadi
(
    id              bigint generated by default as identity -- ID raqam (primary key)
        primary key,
...
create table public.personal_relative -- Ushbu jadvalda hodimlarning qarindoshlari haqida ma'lumotlar saqlanadi (Many to Many Table)
(
    relative_id bigint not null -- qarindosh ID raqami (relatives jadvalining id ustuni bilan bog'langan)
        constraint fk61xlfq4fxwo56o7mjq9cqwmxh
            references public.relatives,
    personal_id bigint not null -- hodim ID raqami (personal jadvalining id ustuni bilan bog'langan)
        constraint fksjo3b7o8hlh5kho1j69bdke9v
            references public.personal
);

alter table public.personal_relative
    owner to postgres;
...
create table public.staff_position -- Ushbu jadvalda tashkilotlarning ishchilarning lavozimlari haqida ma'lumotlar saqlanadi
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
    organization_stir varchar(255), -- tashkilot STIR raqami
