...
create table public.staff_position -- Hodimlar lavozimi va shtatlar haqidagi jadval. Tashkilotlardagi shtatlar sonini aniqlash uchun har doim qat'iy ravishda ushbu jadvalning personal_limit ustunidan tashkilotdagi ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak. ushbu query orqali chiqariladi: SELECT (SELECT SUM(sp.personal_limit) FROM public.staff_position sp WHERE sp.organization_stir = '200941518') - (SELECT COUNT(DISTINCT p.id) FROM public.personal p WHERE p.organization_stir = '200941518' AND p.status ILIKE '%Bo`shatilgan%'
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
    ...
    department_id     bigint -- bo'lim ID raqami (department jadvalining id ustuni bilan bog'langan)
        constraint fk2rccfqg11kksq37ku44aforg5
            references public.department,
    personal_limit    integer, -- shtat bo'yicha limit (tashkilotdagi shtatlar soni so'ralganda ushbu ustunni yig'indisidan ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak)
