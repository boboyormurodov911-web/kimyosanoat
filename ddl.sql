...
(
    ...
    account_number   varchar(255), -- hisob raqami, kartoteka ma'lumotlarini olmaslik kerak, majburiy! (har doim qat'iy ravishda qoldiqlar haqidagi ma'lumotlar so'ralganda kartoteka ma'lumotlari yani account_number usutnidagi 9632 raqami bilan boshlangan qiymatlar olinmaydi(account_number NOT ILIKE '9632%'))
    date             date, -- sana
...
create table public.staff_position -- Hodimlar lavozimi va shtatlar haqidagi jadval. Zavodlardagi shtatlar sonini aniqlash uchun har doim qat'iy ravishda zavoddagi ishdan bo'shatilgan hodimlar sonini hisoblab chiqarib ushbu jadvalning personal_limit ustunidan ayirib chiqarish kerak: SELECT sum(sp.personal_limit) - (select count(*) from personal p where p.status = 'Bo`shatilgan') FROM staff_position AS sp. Ushbu example queryda hech qanaqa JOIN amalga oshirish kerak emas!
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
