...
create table public.staff_position -- Ushbu jadvalda ishchilarning lavozimlari haqida ma'lumotlar saqlanadi (Tashkilotlardagi shtatlar sonini aniqlash uchun ushbu jadvalning personal_limit dan tashkilotdagi ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak -> SELECT SUM(sp.personal_limit) - (SELECT COUNT(DISTINCT p.id) FROM public.personal p WHERE p.status ILIKE '%Bo`shatilgan%') FROM staff_position sp; (hech qanaqa department jadvali yoki ustunini tekshirish shart emas!)
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
