...
create table public.staff_position -- Ushbu jadvalda ishchilarning lavozimlari haqida ma'lumotlar saqlanadi (Tashkilotlardagi shtatlar sonini aniqlash uchun har doim qat'iy ravishda ushbu jadvalning personal_limit dan tashkilotdagi ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak. ushbu query orqali chiqariladi: SELECT (SELECT SUM(sp.personal_limit) FROM public.staff_position sp WHERE sp.organization_stir = '200941518') - (SELECT COUNT(DISTINCT p.id) FROM public.personal p WHERE p.organization_stir = '200941518' AND p.status ILIKE '%Bo`shatilgan%');
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
