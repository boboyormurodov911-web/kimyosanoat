...
create table public.personal -- Ushbu jadvalda hodimlarning shaxsiy ma'lumotlari saqlanadi (Tashkilotlar bo'yicha hodimlar sonini chiqarish uchun hisoblash kerak -> select count(personal_id from staff_position_personal join staff_position as sp on staff_position_personal.staff_position_id = sp.id join personal as p on staff_position_personal.personal_id = p.id where sp.organization_stir = '200002933' and p.status = 'Ish faoliyatida';)
