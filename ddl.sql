...
create table public.organization_bank_account_saldos -- Ushbu jadvalda tashkilot(zavod)larning bank hisoblari bo'yicha qoldiq summalari saqlanadi
(
    id               bigint       not null -- ID raqam (primary key)
        primary key,
    datetime_created timestamp(6) not null, -- ma'lumot yaratilgan sana va vaqt
...
create table public.personal -- Ushbu jadvalda hodimlarning shaxsiy ma'lumotlari saqlanadi (Tashkilotlar bo'yicha hodimlar sonini chiqarish uchun hisoblash kerak -> COUNT(personal.id) va personal.status == "Ish faoliyatida" va staff_position_personal jadvalidan foydalanish kerak)
from staff_position_personal
         join staff_position as sp on staff_position_personal.staff_position_id = sp.id
         join personal as p on staff_position_personal.personal_id = p.id
where sp.organization_stir = '200002933'
  and p.status = 'Ish faoliyatida'; )
(
    ...
    organization_stir       varchar(255), -- hodimning tashkiloti STIR(inn) raqami(Tashkilotlar bo'yicha hodimlar sonini chiqarish uchun staff_position jadvalidagi organization_stir ustunidan foydalanish kerak)
...
create table public.relatives -- Ushbu jadvalda hodimlarning qarindoshlari haqida ma'lumotlar saqlanadi (hodimlarning qarindoshlarini tanlashda ushbu shart bilan tanlash kerak -> is_deleted = false)
(
    id              bigint generated by default as identity -- ID raqam (primary key)
        primary key,
    ...
    kinship         varchar(255) -- qarindoshlik darajasi (OTA, ONA, OZI, ER, XOTIN, FARZAND, AYOL, QAYNOTA, QAYNONA, QAYN_AKA_UKA, AKA_UKA, QAYN_OPA_SINGIL, OPA_SINGIL)
...
<<<<<<< HEAD
create table public.staff_position -- Ushbu jadvalda ishchilarning lavozimlari haqida ma'lumotlar saqlanadi (Tashkilotlar bo'yicha hodimlar sonini chiqarishda uchun staff_position jadvalidagi organization_stir ustunidan foydalanish kerak va staff_position_personal jadvalidan foydalanish kerak)
=======
create table public.staff_position -- Ushbu jadvalda ishchilarning lavozimlari haqida ma'lumotlar saqlanadi (Tashkilotlardagi shtatlar sonini aniqlash uchun ushbu jadvalning personal_limit dan tashkilotdagi ishdan bo'shatilgan hodimlar sonini ayirib chiqarish kerak -> SELECT SUM(sp.personal_limit) - (SELECT COUNT(DISTINCT p.id) FROM public.personal p WHERE p.status ILIKE '%Bo`shatilgan%') FROM staff_position sp)
>>>>>>> 0c551370e4b1a0c94ccdd98455810edd01aecc3f
(
    id                bigint generated by default as identity -- ID raqam (primary key)
        primary key,
    organization_stir varchar(255), -- tashkilot STIR(inn) raqami (200599579 = 'Ammofos-Maxam', 206887857 = 'Dehqonobod kaliy zavodi', 200941518 = 'Maxam-Chirchiq', 200002933 = 'Navoiyazot', 203621367 = 'O'zkimyosanoat', 309341717 = 'Qizilqum fosforit kompleksi') 
